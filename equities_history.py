...
class EquityHistory(db.Model):
	...
	# init users table
	__tablename__ = 'equities_history'
	...
	def add_new(_security, _market, _sector, _previous_open_price, _open_price, _close_price):
		# pass
		equity = EquityHistory(security=_security, market=_market, sector=_sector, previous_open_price=_previous_open_price, open_price=_open_price, close_price=_close_price)
	...
	# get all equity
	def get_all():
		equities = db.session.query(EquityHistory).all()
		return [EquityHistory.json(equity) for equity in equities]
